(() => {
  ...
  async function waitForGone(selector, baseEl, timeoutMs) {
    if (timeoutMs === undefined) timeoutMs = DEFAULT_ELEMENT_TIMEOUT_MS;
    if (baseEl === undefined) baseEl = document;
    ...
    debugLog(`did not disappear: ${selector} inside`, baseEl);
    return false;
  }
  ...
  function isElementClickable(el) {
    ...
    // If element (or parent) is hidden
    const style = window.getComputedStyle(el);
    if (style && (style.display === 'none' || style.visibility === 'hidden' || style.opacity === '0')) return false;
    return true;
  }

  function findCloseButton(baseEl) {
    if (!baseEl) return null;

    // Prefer the new Close button you pasted:
    // <ytcp-button-shape><button aria-label="Close"> ... Close ... </button>
    const candidates = [
      ...baseEl.querySelectorAll('ytcp-button-shape > button[aria-label="Close"]'),
      ...baseEl.querySelectorAll('button[aria-label="Close"]'),
    ];

    // Prefer ones that actually show "Close" text (avoid random aria-label reuse)
    const scored = candidates
      .map((btn) => {
        const txt = (btn.textContent || '').trim().toLowerCase();
        const score =
          (txt === 'close' ? 100 : txt.includes('close') ? 50 : 0) +
          (btn.matches('ytcp-button-shape > button[aria-label="Close"]') ? 25 : 0);
        return { btn, score };
      })
      .sort((a, b) => b.score - a.score)
      .map((x) => x.btn);

    const best = scored.find(isElementClickable);
    return best || null;
  }
  ...
  // Old close icon selector (keep as fallback)
  const DIALOG_CLOSE_ICON_SELECTOR = 'tp-yt-iron-icon';
  ...
  class SuccessDialog {
    ...
    async closeButton() {
      // First try the new "Close" button inside the dialog
      let btn = findCloseButton(this.raw);
      if (btn) return btn;

      // Fallback to the old icon-based close
      const icon = await waitForElement(DIALOG_CLOSE_ICON_SELECTOR, this.raw, 1500);
      return icon;
    }
    ...
    async close() {
      ...
      if (!btn) {
        debugLog('SuccessDialog: no close control found');
        return;
      }
      click(btn);
      await sleep(100);
      // Wait for the share dialog wrapper to be gone if possible
      await waitForGone(DIALOG_SELECTOR, document, 8000);
      debugLog('SuccessDialog: closed');
    }
  }
  ...
  class DraftModal {
    ...
    async closeButton() {
      // Try within the modal first
      let btn = findCloseButton(this.raw);
      if (btn) return btn;

      // Fallback: sometimes the close button is outside the modal wrapper
      btn = findCloseButton(document);
      return btn;
    }
    ...
    async close() {
      const btn = await this.closeButton();
      if (!btn) {
        debugLog('DraftModal: Close button not found');
        return;
      }
      click(btn);
      await sleep(150);

      // Wait for the draft modal to actually disappear before moving on
      await waitForGone(DRAFT_MODAL_SELECTOR, document, 10000);
      debugLog('DraftModal: closed');
    }
  }
  ...
  async function publishDrafts() {
    ...
    for (let video of videos) {
      ...
      // ✅ NEW: close the draft modal before moving to next row
      await draft.close();
  ...
  async function playlistVideos() {
    return [...document.querySelectorAll('ytd-playlist-video-renderer')].map((el) => new PlaylistVideo(el));
  }
